...
suite.addBatch({
    "crossfilter": {
        ...
        "up to 32 dimensions supported": function () {
            var data = crossfilter([]);
            for (var i = 0; i < 32; i++) data.dimension(function () {
                return 0;
            });
        ...
        "can add and remove 32 dimensions repeatedly": function () {
            var data = crossfilter([]),
                dimensions = [];
            for (var j = 0; j < 10; j++) {
                for (var i = 0; i < 32; i++) dimensions.push(data.dimension(function () {
                    return 0;
                }));
            ...
            "filterRange": {
                "selects records greater than or equal to the inclusive lower bound": function (data) {
                    try {
                        ...
                        assert.isTrue(data.date.top(Infinity).every(function (d) {
                            return d.total >= 100;
                        }));
                ...
                "dispose": {
                    "detaches from reduce listeners": function () {
                        var data = crossfilter([0, 1, 2]),
                            ...
                            all = dimension.groupAll().reduce(function () {
                                callback = true;
                            }, function () {
                                callback = true;
                            }, function () {
        ...
        "groupAll": {
            topic: function (data) {
                data.all = data.groupAll().reduceSum(function (d) {
                    return d.total;
                });
                return data;
            },
            ...
            "dispose": {
                "detaches from reduce listeners": function () {
                    var data = crossfilter([0, 1, 2]),
                        ...
                        all = data.groupAll().reduce(function () {
                            callback = true;
                        }, function () {
                            callback = true;
                        }, function () {
        ...
        "add": {
            ...
            "can add more than 256 groups": function (data) {
                ...
                assert.deepEqual(foos.all().map(function (d) {
                    return d.key;
                }), d3.range(0, 256));
                assert(foos.all().every(function (d) {
                    return d.value == 1;
                }));
                data.add([128]);
